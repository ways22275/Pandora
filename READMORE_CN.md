...
1. 实现 `tech.kw.pandora.function.IFunc` 接口，返回相应的Icon、Name以及触发操作：
...
1. 实现 `tech.kw.pandora.inspector.attribute.IParser` 接口并指定所关注的View类型，这里以已经实现的ImageView为例：
```
...
1. 实现 `tech.kw.pandora.preference.protocol.IProvider`接口，返回对应的文件列表：
