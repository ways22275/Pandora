...
1. Implement `tech.kw.pandora.function.IFunc` , return the icon, name and the action：
...
1. implement `tech.kw.pandora.inspector.attribute.IParser` interface and specify the type of View that you are interested in. Here is an example of an already implemented ImageView：
```
...
1. implement `tech.kw.pandora.preference.protocol.IProvider` interface，and return the corresponding file list：
